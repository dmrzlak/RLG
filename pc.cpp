...
void equip_item(pc *p, object *o, int equip_ind){
  if(equip_ind<0 || equip_ind>=12){
    //Silly you, you're trying to equip something that doesn't exist
  }
  else{
    if(p->equip[equip_ind]){
      //item already equipped in slot time to swap them. Assume that the pc
      // knows best (they usually don't) Who cares if it makes them weaker
      object *temp=p->equip[equip_ind];
      int speed=p->speed;
      speed=speed-temp->get_speed()+o->get_speed();
      p->speed=speed;
      p->equip[equip_ind]=o;
      for(int i=0; i<10; i++){
	if(p->inventory[i]==o){
