...
void io_display_equip(dungeon *d){
  ...
  mvprintw(3, 19, " %-40s ", "");
  /* Borrow the first element of our array for this string: */
  mvprintw(4, 19, "You are wearing these items");
  mvprintw(5, 19, " %-40s ", "");
  refresh();
  for (i = 0; i < 12; i++) {
    char c=i+'a';
    ...
    mvprintw(i + 6, 19, "%c) %s-%s",c, type.c_str(), 
	     d->PC->equip[i] ? d->PC->equip[i]->get_name(): "");
  }
  refresh();
  int key;
  do{
...
void io_display_inventory(dungeon *d){
  ...
  mvprintw(3, 19, " %-40s ", "");
  /* Borrow the first element of our array for this string: */
  mvprintw(4, 19, "You are holding these items");
  mvprintw(5, 19, " %-40s ", "");
  refresh();
  for (i = 0; i < 10; i++) {
    char c=i+'0';
    std::string type;
    if(d->PC->inventory[i]){
      switch(d->PC->inventory[i]->get_type()){
      case 1:
	type="Weapon";
	break;
      case 2:
...
uint32_t i;
  ...
  refresh();
  mvprintw(16, 19, "Please select an item 0-9, Press esc to cancel");
  int index;
  int fail_code=1;
  int key;
  do{
	...
	if(d->PC->inventory[index]!=NULL){
	  slot=d->PC->inventory[index]->get_equip_index();
	  if(slot==10){
	    mvprintw(16, 19, "You selected a ring. Choose ring slot, or esc");
	    refresh();
	    int yes=0;
	    do{
	      key=getch();
		...
		mvprintw(16, 19, 
			 "Please select a proper item or hit esc to cancel");
		refresh();
	      }
	    }while(!yes);
	  }
	  equip_item(d->PC, d->PC->inventory[index],slot);
	  
	  fail_code=0;
	}
	else{
	  mvprintw(16, 19, "Please select a proper item or hit esc to cancel");
	  refresh();
	}
      }
      else{
...
uint32_t i;
  ...
  do{
    ...
    if(key==27){
      ...
      if(index>=0 && index<=9){
	if(d->PC->inventory[index]!=NULL){
	  objpair(d->PC->position)=d->PC->inventory[index];
...
void io_destroy_item(dungeon *d){
  io_drop_item(d);
  objpair(d->PC->position)=NULL;
}
void io_unequip_item(dungeon *d){
  uint32_t i;
  int speed=d->PC->speed;
  mvprintw(3, 19, " %-40s ", "");
  /* Borrow the first element of our array for this string: */
  mvprintw(4, 19, "You are wearing these items");
  mvprintw(5, 19, " %-40s ", "");
  ...
  refresh();
  mvprintw(18, 19, "Please select an item a-l, Press esc to cancel");
  int index;
  int fail_code=1;
  int key;
  do{
    ...
    if(key==27){
      ...
      index=key-'a';
      if(index>=0 && index<=11){
	if(d->PC->equip[index]!=NULL){
	  for(int i=0; i<10; i++){
	    if(d->PC->inventory[i]==NULL){	 
	       d->PC->inventory[i]=d->PC->equip[index];
	       speed-=d->PC->equip[index]->get_speed();
	       d->PC->speed=speed;
	  ...
	  mvprintw(18, 19, "Please select a proper item or hit esc to cancel");
	  refresh();
	}
      }
      else{
...
void io_inspect_item(dungeon *d){
  object *inspect=NULL;
  mvprintw(4, 19, "Would you like to inspect your equipment or inventory?");
  mvprintw(5, 19, "e for equipment. i for inventory");
  refresh();
  int fail_code=1;
  do{
    int key=getch();
    ...
    if(key-'e'==0){
      //display equipment then let them select
      uint32_t i;
	...
	if(key==27){
      ...
      mvprintw(6, 19, "Press e or i");
    }
  }while(fail_code);
  io_display(d);
  if(inspect!=NULL){
    mvprintw(3, 19, "Hit esc to continue");
    mvprintw(4, 19, "Inspecting Object %s", inspect->get_name());
    mvprintw(5, 19, "Speed: %d", inspect->get_speed());
    mvprintw(6, 0, "Description: %s", inspect->get_description().c_str());
    int key;
    refresh();
    do{
    }while( (key=getch())!=27);
...
void io_inspect_character(dungeon *d){
  pair_t dest;
  int c;
  fd_set readfs;
  struct timeval tv;
  ...
  mvprintw(0, 0, "Choose a location.  't' select character; esc to abort.");
  ...
  * not otherwise used.                                                 */
  ...
  if(c==27){
    ...
    if(charpair(dest)){
      //inspect the character, I will let the player look at himself
      //such a narcissist
      character *cur=charpair(dest);
      io_display_no_fog(d);
      ...
      mvprintw(3, 19, "%s", cur->name);
      mvprintw(4, 19, "%s%d", "HP: ", cur->hp);
      mvprintw(5, 19, "%s%d+%dd%d", "Damage: ", cur->damage->get_base(), 
	       cur->damage->get_number(), cur->damage->get_sides());
      mvprintw(6, 19, "%s%d", "Speed: ", cur->speed);
      // mvprintw(7, 19, "%s%s", "Description: ", cur->description);
      refresh();
      do{
      }while( (c=getch() ) != 27);
    }
    else{
      mvprintw(0, 0, "Selection failed, no character in position");
    }
  }
  io_display(d);
}
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 's':
      ...
      break;
    case 'L':
      //New command. Look at a monster
      io_inspect_character(d);
      fail_code = 1;
      break;
    case 'g':
