...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if(atk==d->PC){
    ...
    for(int i=0; i<12; i++){
      if(d->PC->equip[i]){
	object *cur=d->PC->equip[i];
	damage+=cur->roll_dice();
      }
    }
    hp=( hp ) - (damage);
    if(hp<=0){
      ...
      io_queue_message("You smite %s%s!", 
		       is_unique(def) ? "" : "the ", def->name);
      d->num_monsters--;
    }
  }
  else{
    //attacker is not the pc
    hp=( hp ) - ( atk->damage->roll() );
    if(hp<=0){
      //PC is dead
      d->PC->alive=0;
      charpair(def->position)=NULL;
...
{
  if (c==d->PC && charpair(next) &&
      ((next[dim_y] != c->position[dim_y]) ||
       (next[dim_x] != c->position[dim_x]))) {
    do_combat(d, c, charpair(next));
  }
  else if (charpair(next) && charpair(next)==d->PC &&
      ((next[dim_y] != c->position[dim_y]) ||
       (next[dim_x] != c->position[dim_x]))) {
    do_combat(d, c, charpair(next));
  }else if( charpair(next) &&
      ((next[dim_y] != c->position[dim_y]) ||
       (next[dim_x] != c->position[dim_x]))) {
    //swap the monsters positions
    d->character_map[c->position[dim_y]][c->position[dim_x]] = charpair(next);
    charpair(next)->position[dim_y]=c->position[dim_y];
    charpair(next)->position[dim_x]=c->position[dim_x];
    c->position[dim_y] = next[dim_y];
    c->position[dim_x] = next[dim_x];
    d->character_map[c->position[dim_y]][c->position[dim_x]] = c;
  }else if (c==d->PC && objpair(next) && objpair(next)->get_type()!=0){
    //PC will land on an object
    for(int i=0; i<10; i++){
      if(d->PC->inventory[i]==NULL){
	d->PC->inventory[i]=objpair(next);
	int index= objpair(next)->get_equip_index();
	if(index==10 && d->PC->equip[index]!=NULL){
	  index++;
	}
	if(d->PC->equip[index]==NULL){
	equip_item(d->PC, d->PC->inventory[i], index);
	}
	objpair(next)=NULL;
	break;
      }
